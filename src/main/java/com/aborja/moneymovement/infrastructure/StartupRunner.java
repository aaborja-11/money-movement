...
public class StartupRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        final var newBusiness = Asset.builder().name("My Furry Paws")
                .updatedBy("TESTTTTING!!").build();
        //newBusiness.setId(businessId);
        //newBusiness.setVersion(0L);

        final var business = assetRepository.findById(UUID.fromString("9fa20f34-0cb3-4f42-962e-02dd211e5e13")).orElse(newBusiness);
        business.setName("Updated Name 1");
        business.setCreatedBy("TEST");
        final var saved = assetRepository.save(business);
        int x = 1;
    }
}
